...
$.gulp.task( 'compile:js', function () {
  var dirname = '';
  return $.gulp.src( init.source.path.root + init.source.path.js )
    .pipe( $.rename( function ( path ) {
      dirname = path.dirname;
      if ( dirname.indexOf( 'clientlibs' ) < 0 ) {
        $.exit();
      }
    } ) )
...
$.gulp.task( 'compile:sass', function () {
  var dirname = '';
  return $.gulp.src( init.source.path.root + init.source.path.scss )
    .pipe( $.rename( function ( path ) {
      dirname = path.dirname;
      if ( dirname.indexOf( 'clientlibs' ) < 0 ) {
        $.exit();
...
$.gulp.task( 'compile:less', function () {
  var dirname = '',
    task = [];
  return $.gulp.src( init.source.path.root + init.source.path.less )
    .pipe( $.rename( function ( path ) {
      dirname = path.dirname;
      if ( dirname.indexOf( 'clientlibs' ) < 0 ) {
        $.exit();
...
$.gulp.task( 'compile:all', [ 'compile:js', 'compile:sass', 'compile:less' ] );
